...
export function validateRegisterRequest({ body }) {
  ...
  if (body.password !== body.confirmPassword) errors.push(createValidationError("confirmPassword", "Mật khẩu xác nhận không khớp.", "body", "PASSWORD_CONFIRMATION_MISMATCH"));
...
export function validateVerifyOtpRequest({ body }) {
  ...
  if (body.password || body.confirmPassword) { results.push(validatePassword(body.password, { required: true, strong: true })); if (body.password !== body.confirmPassword) errors.push(createValidationError("confirmPassword", "Mật khẩu xác nhận không khớp.", "body", "PASSWORD_CONFIRMATION_MISMATCH")); }
...
export function validateResetPasswordRequest({ body }) {
  ...
  if (body.password !== body.confirmPassword) {
    errors.push(createValidationError("confirmPassword", "Mật khẩu xác nhận không khớp.", "body", "PASSWORD_CONFIRMATION_MISMATCH"));
  }
