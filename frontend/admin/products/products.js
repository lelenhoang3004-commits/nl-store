...
function renderRow(product) {
  ...
  const overflowActions = [
    canUpdate ? `<button type="button" class="product-action-menu-item is-stock" data-product-stock="${id(product.id)}"><i class="fa-solid fa-boxes-stacked" aria-hidden="true"></i><span>Cập nhật kho</span></button>` : "",
    canUpdate ? `<button type="button" class="product-action-menu-item is-variants" data-product-variants="${id(product.id)}"><i class="fa-solid fa-layer-group" aria-hidden="true"></i><span>Quản lý biến thể</span></button>` : "",
    canUpdate && product.status !== "out_of_stock" ? `<button type="button" class="product-action-menu-item is-visibility" data-product-id="${id(product.id)}" data-product-status="${nextStatus}"><i class="fa-regular fa-eye-slash" aria-hidden="true"></i><span>${nextStatus === "active" ? "Hiện" : "Ẩn"}</span></button>` : "",
    canDelete ? `<button type="button" class="product-action-menu-item is-danger" data-product-delete="${id(product.id)}"><i class="fa-regular fa-trash-can" aria-hidden="true"></i><span>Xóa</span></button>` : ""
  ].filter(Boolean).join("");
  return `<tr>
