...
function renderRow(product) {
  ...
  return `<tr>
    <td><img class="admin-product-thumb" src="${escapeHtml(resolveImageUrl(product.thumbnailUrl))}" alt="${escapeHtml(product.name)}" data-product-image></td>
    ...
    <td><div class="admin-product-actions"><a href="#products/${id(product.id)}" data-page="products/${id(product.id)}">Xem</a>${canUpdate ? `<button type="button" data-product-edit="${id(product.id)}">Sửa</button><button type="button" data-product-stock="${id(product.id)}">Cập nhật kho</button><button type="button" data-product-variants="${id(product.id)}">Quản lý biến thể</button>${product.status !== "out_of_stock" ? `<button type="button" data-product-id="${id(product.id)}" data-product-status="${nextStatus}">${nextStatus === "active" ? "Hiện" : "Ẩn"}</button>` : ""}` : ""}${canDelete ? `<button type="button" class="is-danger" data-product-delete="${id(product.id)}">Xóa</button>` : ""}</div></td>
  </tr>`;
}
  ...
  const modal = createModal(`
    ...
    <form class="admin-product-form" data-product-form novalidate>
      <div class="admin-product-form-body">
        ...
        <section class="admin-product-form-section"><div class="admin-product-form-section-title"><i class="fa-solid fa-images"></i><div><h3>Ảnh sản phẩm</h3><p>Tải ảnh đại diện từ máy tính và quản lý thư viện ảnh</p></div></div><div class="admin-product-image-layout"><div class="admin-product-upload-column"><input type="hidden" name="thumbnail_url" value="${escapeHtml(product?.thumbnailUrl || "")}"><input class="admin-product-file-input" id="product-thumbnail-file" type="file" multiple accept="image/*" data-product-image-input><div class="admin-product-upload-box" data-product-dropzone tabindex="0" role="button" aria-label="Chọn ảnh sản phẩm từ máy tính"><i class="fa-solid fa-cloud-arrow-up" aria-hidden="true"></i><strong>Kéo thả ảnh vào đây</strong><span>hoặc bấm để chọn ảnh từ máy tính</span><small>Hỗ trợ JPG, PNG, WEBP. Tối đa 5MB.</small><button type="button" data-product-choose-image>Chọn ảnh sản phẩm</button></div><div class="admin-product-upload-status" aria-live="polite"><span data-product-file-name>${product?.thumbnailUrl ? "Ảnh hiện tại" : "Chưa chọn ảnh"}</span><strong data-product-upload-message>${product?.thumbnailUrl ? "Sẵn sàng đổi ảnh" : ""}</strong></div></div><div class="admin-product-thumbnail-card"><div class="admin-product-thumbnail-preview"><img src="${escapeHtml(resolveImageUrl(product?.thumbnailUrl))}" alt="Xem trước ảnh thumbnail" data-thumbnail-preview></div><div><button type="button" class="admin-product-remove-image" data-product-remove-image ${product?.thumbnailUrl ? "" : "hidden"}><i class="fa-regular fa-trash-can"></i> Xóa ảnh</button></div></div><label class="admin-product-gallery-field"><span>Gallery URLs</span><textarea name="gallery_urls" rows="5" placeholder="Mỗi dòng một URL ảnh">${escapeHtml((product?.galleryUrls || []).join("\n"))}</textarea><small class="admin-product-field-hint">Gallery vẫn hỗ trợ mỗi dòng một URL. Tối đa nên dùng 8 ảnh.</small></label></div><div class="admin-product-gallery-preview" data-gallery-preview></div></section>
...
function renderDetail(product) {
  ...
  return `<section class="admin-product-detail"><header class="admin-products-hero"><div><p class="admin-products-eyebrow">Chi tiết sản phẩm</p><h1>${escapeHtml(product.name)}</h1><p>${escapeHtml(product.sku)}</p></div><a href="#products" data-page="products">← Danh sách</a></header><div class="admin-product-detail-grid"><article class="admin-product-card admin-product-detail-image"><img src="${escapeHtml(resolveImageUrl(product.thumbnailUrl))}" alt="${escapeHtml(product.name)}" data-product-image>${statusBadge(product.status)}${stockBadge(product.stock)}</article><article class="admin-product-card"><h2>Thông tin sản phẩm</h2>${detailRow("Danh mục", product.categoryName)}${detailRow("Thương hiệu", product.brand)}${detailRow("Giá", formatCurrency(product.price))}${detailRow("Giá sale", product.salePrice === null ? "-" : formatCurrency(product.salePrice))}${detailRow("Đã bán", product.sold)}${detailRow("Biến thể", product.variantCount || 0)}${detailRow("Slug", product.slug)}${detailRow("Cập nhật", formatDate(product.updatedAt))}<div class="admin-product-detail-actions">${hasProductPermission(PERMISSIONS.PRODUCT_UPDATE) ? `<button type="button" data-detail-edit="${id(product.id)}">Sửa sản phẩm</button><button type="button" data-detail-stock="${id(product.id)}">Cập nhật kho</button><button type="button" data-detail-variants="${id(product.id)}">Quản lý biến thể</button>` : ""}</div></article></div><article class="admin-product-card"><h2>Mô tả ngắn</h2><p>${escapeHtml(product.shortDescription || "-")}</p><h2>Mô tả chi tiết</h2><p class="admin-product-description">${escapeHtml(product.description || "-")}</p><h2>Tags</h2><p>${escapeHtml((product.tags || []).join(", ") || "-")}</p></article></section>`;
}
...
function formPayload(form) {
  ...
  return {
    ...
    price: Number(d.get("price")), sale_price: d.get("sale_price") === "" ? null : Number(d.get("sale_price")), stock: Number(d.get("stock") || 0), status: d.get("status"),
...
function bindProductImageUpload(modal) {
  ...
  const updateGallery = () => {
    ...
    gallery.innerHTML = urls.length ? urls.map((url, index) => `<img src="${escapeHtml(resolveImageUrl(url))}" alt="Ảnh gallery ${index + 1}" data-product-image>`).join("") : '<span>Chưa có ảnh gallery</span>';
